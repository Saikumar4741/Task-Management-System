...
**Regular User** — Register directly at `http://localhost:3000/register`

**Admin User** — Use Postman or any API client:
```
...
> All Task and Admin routes require:
> `Authorization: Bearer YOUR_JWT_TOKEN` in the request header
...
```
> Copy the `token` value — use it as `Bearer TOKEN` in all further requests.
...
#### 3. Get All Tasks (with Filter, Search, Sort, Pagination)
```
GET /api/tasks
```
| Query Param | Accepted Values | Example |
|-------------|----------------|---------|
| `status` | `Todo`, `In Progress`, `Done` | `?status=Todo` |
| `priority` | `Low`, `Medium`, `High` | `?priority=High` |
| `search` | any string | `?search=design` |
| `sortBy` | `dueDate`, `priority` | `?sortBy=dueDate` |
...
Full example:
```
GET /api/tasks?status=Todo&priority=High&sortBy=dueDate&order=asc&page=1&limit=6
```
```json
Response 200:
{
...
> Regular users get `403 Access denied. Admins only.`

#### 8. Get ALL Tasks (from all users)
```
GET /api/admin/tasks
```
```json
